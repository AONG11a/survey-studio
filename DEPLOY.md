...
เสร็จแล้ว! เปิด URL นั้น → สมัครสมาชิกหรือล็อกอิน `demo / demo1234` → สร้างฟอร์ม →
...
1. New + → **PostgreSQL** → plan **Free** → Create (ฐานข้อมูลฟรี 1 GB)
...
4. ไปที่ **Settings → Build Command** เปลี่ยนเป็น:
   ```
   pip install -r requirements.txt psycopg2-binary
   ```
   (เพิ่มไดรเวอร์ Postgres — โค้ดจะสลับไปใช้ Postgres อัตโนมัติเมื่อมี `DATABASE_URL`)
5. Save → deploy ใหม่ ตอนนี้คำตอบจะถูกเก็บถาวรใน Postgres แล้ว
   > หมายเหตุ: Postgres ฟรีของ Render มีอายุ ~30 วันหลังสร้าง หลังจากนั้นต้องอัปเกรดเป็นแบบจ่ายเงินเพื่อไม่ให้ข้อมูลหาย
